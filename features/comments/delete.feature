...
Feature: ログインが必要な操作
  ...
  Scenario: 自分が投稿したお宝情報を削除
    ...
    When I am logged in as "localinfo_valid_token"
    And I send a DELETE request to "localinfos/{localinfo_id}/comments/{comment_id}"
  ...
  Scenario: ログインしていない場合は削除できない
    ...
    And I send a DELETE request to "localinfos/1/comments/1"
    Then the response status should be "403"

  @failure @delete
  Scenario: 自分のものでなければ削除できない
    When I am logged in as "localinfo_valid_token"
    And I send a DELETE request to "localinfos/1/comments/2"
    Then the response status should be "403"
  ...
  Scenario Outline: PUTの挙動を確認
    ...
    When I grab "$['result'][0]['localinfo_id']" as "localinfo_id"
    And I am logged in as "localinfo_valid_token"
    And I set form request body to:
      | comment | うん。これで更新できたね。 |
    And I send a PUT request to "localinfos/{localinfo_id}/comments/{comment_id}"
    Then the response status should be "200"
    When I grab "$['success']" as "success"
  ...
  Scenario: ログインせずには更新できない
    ...
    And I set form request body to:
      | comment | うん。これで更新できたね。 |
    And I send a PUT request to "localinfos/1/comments/1"
    Then the response status should be "403"
